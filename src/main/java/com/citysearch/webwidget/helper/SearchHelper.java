...
public class SearchHelper {
	...
	private void validateClosestLocationPostalCodeRequest(SearchRequest request)
		...
		if (!errors.isEmpty()) {
			throw new CitysearchException(this.getClass().getName(),
					"validateClosestLocationPostalCodeRequest",
	...
	private void validateLatitudeLongitudeRequest(SearchRequest request)
		...
		if (!errors.isEmpty()) {
			throw new CitysearchException(this.getClass().getName(),
					"validateLatitudeLongitudeRequest", "Invalid parameters.",
					errors);
		}
	}
	...
	private String getLatitudeLongitudeRequestQueryString(SearchRequest request)
		...
		apiQueryString.append(CommonConstants.SYMBOL_AMPERSAND);
		apiQueryString.append(HelperUtil.constructQueryParam("rpp", request
				.getRpp()));
		apiQueryString.append(CommonConstants.SYMBOL_AMPERSAND);
		apiQueryString.append(HelperUtil.constructQueryParam(
				APIFieldNameConstants.RADIUS, request.getRadius()));
	...
	public String[] getLatitudeLongitude(SearchRequest request)
		...
		try {
			...
			throw new CitysearchException(this.getClass().getName(),
					"getLatitudeLongitude", ihe.getMessage());
		}
