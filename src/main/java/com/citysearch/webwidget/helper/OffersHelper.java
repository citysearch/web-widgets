...
public class OffersHelper {
     ...
     */
    private String getQueryString(OffersRequest request)
            throws CitysearchException {
        log.info("Start offersHelper getQueryString()");
        StringBuilder strBuilder = new StringBuilder();
        /*
         * strBuilder.append(HelperUtil.constructQueryParam(APIFieldNameConstants
    ...
    public List<Offer> getOffers(OffersRequest request)
        ...
        try {
            ...
            responseDocument = HelperUtil.getAPIResponse(urlString.toString(),
                    hdrMap);
        ...
        if (offersList != null && !offersList.isEmpty()) {
            for (Offer offer : offersList) {
                ...
                if (profile != null) {
                    ...
                    StringBuilder couponUrl = new StringBuilder(profile
                            .getProfileUrl());
     ...
     * @throws CitysearchException
     */
    private List<Offer> parseXML(OffersRequest request, Document doc)
            throws CitysearchException {
        ...
        if (doc != null && doc.hasRootElement()) {
            ...
            if (StringUtils.isBlank(request.getLatitude())
                    || StringUtils.isBlank(request.getLongitude())) {
                offersList = (List<Offer>) getOffersByRating(request,
                        offersElementList);
            } else {
                offersList = (List<Offer>) getOffersByDistance(request,
                        offersElementList);
            }
        }
    ...
    private List<Offer> getOffersByDistance(OffersRequest request,
            List<Element> offerElemList) throws CitysearchException {
    ...
    private List<Offer> getOffersByRating(OffersRequest request,
            List<Element> offerElemList) throws CitysearchException {
    ...
    private Offer toOffer(OffersRequest request, Element offerElement) {
        ...
        if (!StringUtils.isBlank(request.getLatitude())
            ...
            BigDecimal sourceLatitude = new BigDecimal(request.getLatitude());
            BigDecimal sourceLongitude = new BigDecimal(request.getLongitude());
            BigDecimal businessLatitude = new BigDecimal(offerElement
                    .getChildText(CommonConstants.LATITUDE));
            BigDecimal businessLongitude = new BigDecimal(offerElement
                    .getChildText(CommonConstants.LONGITUDE));
            double distance = HelperUtil.getDistance(sourceLatitude,
                    sourceLongitude, businessLatitude, businessLongitude);
        ...
        String location = HelperUtil.getLocationString(offer.getCity(), offer
                .getState());
        ...
        offer.setReviewCount(HelperUtil.toInteger(offerElement
                .getChildText(REVIEW_COUNT)));
        ...
        if (offerTitle != null && offerTitle.trim().length() > OFFER_TITLE_SIZE) {
            offer.setOfferShortTitle(StringUtils.abbreviate(offerTitle,
                    OFFER_TITLE_SIZE));
        } else {
