...
public class GrouponProxy extends AbstractProxy {
	...
	private String getGrouponQueryString(RequestBean request)
		...
		apiQueryString.append(constructQueryParam(GROUPON_LONGITUDE, request
				.getLongitude()));
	...
	private GrouponResponse toOffer(Element dealElm) {
		...
		Element conditionElm = dealElm.getChild("conditions");
	...
	private List<GrouponResponse> parse(Element rootElement,
			int requiredNoOfOffers) {
	...
	public List<GrouponResponse> getOffers(RequestBean request,
		...
		try {
			grouponResponse = getAPIResponse(urlStringBuilder.toString(),
					headers);
