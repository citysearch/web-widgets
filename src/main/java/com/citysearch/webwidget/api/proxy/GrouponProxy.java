...
public class GrouponProxy extends AbstractProxy {
    ...
    private void validate(RequestBean request) throws InvalidRequestParametersException {
        ...
        if (!errors.isEmpty()) {
            throw new InvalidRequestParametersException(this.getClass().getName(),
                    "RequestBean.validate()", "Invalid parameters.", errors);
        }
    ...
    private GrouponResponse toOffer(Element dealElm) {
        ...
        offer.setSoldOut(Boolean.parseBoolean(dealElm.getChildText("sold_out")));
        ...
        if (conditionElms != null && !conditionElms.isEmpty()) {
            ...
            for (Element conditionElm : conditionElms) {
                ...
                condition.setLimitedQuantity(Boolean.parseBoolean(conditionElm.getChildText("limited_quantity")));
                condition.setInitialQuantity(conditionElm.getChildText("initial_quantity"));
                condition.setQuantityRemaining(conditionElm.getChildText("quantity_remaining"));
                condition.setMinimumPurchase(conditionElm.getChildText("minimum_purchase"));
                condition.setMaximumPurchase(conditionElm.getChildText("maximum_purchase"));
                condition.setExpirationDate(conditionElm.getChildText("expiration_date"));
    ...
    private List<GrouponResponse> parse(Element rootElement, int requiredNoOfOffers) {
        ...
        List<Element> deals = dealsElm.getChildren("deal");
    ...
    public List<GrouponResponse> getOffers(RequestBean request, int requiredNoOfOffers)
