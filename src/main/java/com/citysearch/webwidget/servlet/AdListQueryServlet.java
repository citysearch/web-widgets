...
import com.citysearch.webwidget.helper.PfpResponseHelper;
import com.citysearch.webwidget.helper.RequestHelper;
import com.citysearch.webwidget.helper.ResponseHelper;
...
public class AdListQueryServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			// If the request is not valid, user will be directed to an error
			// page
			if (reqHelper.validateRequest()) {
				throw new CitysearchException(this.getClass().getName(),
						"doGet", "Invalid parameters");
			}
			// Calling Search API to get latitude and longitude
			if (!reqHelper.validateLatLon()) {
				querySearchAPI(req, res);
				sourceLatLon[0] = (String) req
						.getAttribute(CommonConstants.LATITUDE);
				sourceLatLon[1] = (String) req
						.getAttribute(CommonConstants.LONGITUDE);

			} else {
				sourceLatLon = reqHelper.getSourceLatLon();
			}
			if (StringUtils.isBlank(sourceLatLon[0])
					|| StringUtils.isBlank(sourceLatLon[1])) {
				String errMsg = PropertiesLoader.getErrorProperties()
						.getProperty(latLonError);
				log.error(errMsg);
			}
			executeRequest(reqHelper, CommonConstants.PFP_API_TYPE, req, res,
					apiProperties, sourceLatLon);
	...
	/**
	 * Sets the default redirect url in a session If any error occurs,this will
	 * be the url the user will be forwarded
	 * 
	 * @param req
	 * @param apiProperties
	 */
	private void setDefaultURL(HttpServletRequest req, Properties apiProperties) {
		String defaultRedirectURL = apiProperties
				.getProperty(CommonConstants.REDIRECT_URL_PARAM);
		HttpSession session = req.getSession(true);
		session.setAttribute(CommonConstants.REDIRECT_URL_PARAM,
				defaultRedirectURL);
	}

	/**
	 * Reads the api properties file. If file is not found, an exception will be
	 * thrown
	 * 
	 * @return
	...
	private Properties getAPIProperties() throws CitysearchException {
		...
		try {
			...
			String errMsg = errorProperties
					.getProperty(CommonConstants.API_PROP_READ_ERROR);
			log.error(errMsg);
			throw new CitysearchException(this.getClass().getName(),
					"getAPIProperties", errMsg);
		}
	...
	private void querySearchAPI(HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException, CitysearchException {
		RequestDispatcher dispatcher = req
				.getRequestDispatcher(getAPIProperties().getProperty(
						searchPagePath));
		dispatcher.include(req, res);
	}

	/**
	 * Calls the pfp api and processes response If pfp api does not return any
	 * result, then a call to pfp api is again issued but, without geography
	 * parameters(where,lat/lon) If no results are returned then search api is
	 * queried and results are returned
	 * 
	 * @param reqHelper
	 * @param apiType
	 * @param req
	 ...
	 * @throws IOException
	 */
	private void executeRequest(RequestHelper reqHelper, String apiType,
			HttpServletRequest req, HttpServletResponse res,
			Properties apiProperties, String[] sourceLatLon)
			throws ServletException, IOException, CitysearchException {
		String url = reqHelper.getQueryString(apiType);
		boolean response = executeRequest(url, req, res, sourceLatLon);
		if (!response) {
			url = reqHelper
					.getQueryString(CommonConstants.PFP_WITHOUT_GEOGRAPHY);
			response = executeRequest(url, req, res, sourceLatLon);
			if (!response) {
				String queried = (String) req
						.getAttribute(CommonConstants.SEARCH_API_QUERIED);
				if (StringUtils.isBlank(queried)) {
					querySearchAPI(req, res);
				}
				Document doc = (Document) req
						.getAttribute(CommonConstants.SEARCHRESPONSE);
				...
				if (!response) {
					String redirectURL = (String) req
							.getParameter(redirectURLParam);
	 ...
	 * @throws ServletException
	 */
	private boolean executeRequest(String url, HttpServletRequest req,
			HttpServletResponse res, String[] sourceLatLon)
			throws ServletException, IOException, CitysearchException {
		...
		try {
			...
			throw new CitysearchException(this.getClass().getName(),
					"executeRequest", excep.getMessage());
		} finally {
			if (connection != null) {
	...
	/**
	 * Processes the response received from host.If any error is received, the
	 * user will be forwarded to a default page.
	 * 
	 * @param req
	 * @param res
	 * @param connection
	...
	private boolean processResponse(HttpServletRequest req,
			HttpServletResponse res, HttpURLConnection connection,
			String[] sourceLatLon) throws CitysearchException,
		...
		try {
			if (connection.getResponseCode() != CommonConstants.RES_SUCCESS_CODE) {
				...
				Document doc = new PfpResponseHelper()
						.getDocumentfromStream(input);
				response = processSuccessResponse(req, res, doc, sourceLatLon,
			...
			log.error(errMsg, excep);
			throw new CitysearchException(this.getClass().getName(),
					"processResponse", excep.getMessage());
		} finally {
			if (input != null) {
				input.close();
			}
	 ...
	 * @throws IOException
	 */
	private boolean processSuccessResponse(HttpServletRequest req,
			HttpServletResponse res, Document doc, String[] sourceLatLon,
			String apiType) throws CitysearchException, ServletException,
			IOException {
		boolean response = false;
		try {
			...
			if (apiType.equalsIgnoreCase(CommonConstants.PFP_API_TYPE)) {
				responseHelper = new PfpResponseHelper();
					...
					.equalsIgnoreCase(CommonConstants.SEARCH_API_TYPE)) {
				responseHelper = new SearchResponseHelper();
			}
			ArrayList<AdListBean> adList = responseHelper.parseXML(doc,
					sourceLatLon[0], sourceLatLon[1], apiType, req
							.getContextPath());
			if (adList.size() > 0) {
				req.setAttribute(resultListParam, adList);
				dispatchRequest(req, res, getAPIProperties().getProperty(
						jspFwdPathParam));
				response = true;
			}
			...
			log.error(errMsg, excep);
			throw new CitysearchException(this.getClass().getName(),
					"processSuccessResponse", excep.getMessage());
		}
		return response;
	}
	...
	private String getRedirectURL(String redirectURL)
		...
		if (StringUtils.isBlank(redirectURL)) {
			redirectURL = getAPIProperties().getProperty(
					CommonConstants.REDIRECT_URL_PARAM);
		}
		return redirectURL;
	}
	...
	private void dispatchRequest(HttpServletRequest req,
			HttpServletResponse res, String resourcePath)
			throws ServletException, CitysearchException {
		try {
			...
			throw new CitysearchException(this.getClass().getName(),
					"dispatchRequest", ioe.getMessage());
		}
	}
