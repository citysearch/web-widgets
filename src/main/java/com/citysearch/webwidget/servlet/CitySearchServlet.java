...
public class CitySearchServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (adUnitName != null) {
			...
			if (adUnitName
					.equalsIgnoreCase(CommonConstants.AD_UNIT_NAME_NEARBY)) {
				if (publisher != null
						&& publisher
								.equals(CommonConstants.PUBLISHER_PROJECT_YELLOW)) {
					strBuilder.append("/nearbyplacesyellow");
				} else {
					strBuilder.append("/nearbyplaces");
				}
			} else if (adUnitName
					.equalsIgnoreCase(CommonConstants.AD_UNIT_NAME_REVIEW)) {
				strBuilder.append("/review");
			} else if (adUnitName
					.equalsIgnoreCase(CommonConstants.AD_UNIT_NAME_OFFERS)) {
				if (adUnitSize != null
