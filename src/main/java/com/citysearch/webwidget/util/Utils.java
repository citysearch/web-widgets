...
public class Utils {
	...
	public static String getTruncatedString(String stringTotruncate,
			String propertyName) throws CitysearchException {
		Properties prop = PropertiesLoader.getFieldProperties();
		if (prop.containsKey(propertyName)) {
			String value = prop.getProperty(propertyName);
			if (StringUtils.isNotBlank(value)) {
				int length = NumberUtils.toInt(value);
				stringTotruncate = StringUtils.substring(stringTotruncate, 0,
						length);
			}
		}
	...
	public static String getThirdPartyTrackingUrl(String urlToTrack,
			String dartTrackingUrl, String thirdPartyTrackingUrlKey)
			throws CitysearchException {
		try {
			StringBuilder strBuilder = new StringBuilder();
			// dart tracking goes first
			...
			String thirdPartyTrackingUrl = apiProperties
					.getProperty(thirdPartyTrackingUrlKey);
			if (thirdPartyTrackingUrl != null
					&& thirdPartyTrackingUrl.trim().length() > 0) {
				StringBuilder destinationUrlBuilder = new StringBuilder(
	...
	public static String parsePhone(String phoneStr) {
		if (phoneStr != null && phoneStr.trim().length() == 10) {
