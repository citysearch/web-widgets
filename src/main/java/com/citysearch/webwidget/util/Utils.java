...
public class Utils {
    ...
    public static String constructQueryParam(String name, String value) throws CitysearchException {
        ...
        if (StringUtils.isNotBlank(value)) {
            ...
            try {
                ...
                throw new CitysearchException("Utils", "constructQueryParam", excep);
            }
        }
    ...
    public static Date parseDate(String dateStr, SimpleDateFormat formatter)
        ...
        try {
            ...
            throw new CitysearchException("Utils", "parseDate", excep);
        }
        return date;
    }
    ...
    public static String getAbbreviatedString(String stringToAbbreviate, String propertyName)
            throws CitysearchException {
        ...
        if (prop.containsKey(propertyName)) {
            ...
            if (StringUtils.isNotBlank(value)) {
                ...
                stringToAbbreviate = StringUtils.abbreviate(stringToAbbreviate, length);
            }
        }
        return stringToAbbreviate;
    }

    public static String getTruncatedString(String stringTotruncate, String propertyName)
            throws CitysearchException {
        Properties prop = PropertiesLoader.getFieldProperties();
        if (prop.containsKey(propertyName)) {
            String value = prop.getProperty(propertyName);
            if (StringUtils.isNotBlank(value)) {
                int length = NumberUtils.toInt(value);
                stringTotruncate = StringUtils.substring(stringTotruncate, 0, length);
            }
        }
    ...
    public static String getTrackingUrl(String adDisplayURL, String adDestinationUrl,
        ...
        try {
            ...
            throw new CitysearchException("Utils", "getTrackingUrl", mue);
        } catch (UnsupportedEncodingException excep) {
            throw new CitysearchException("Utils", "getTrackingUrl", excep);
        }
    }

    public static String getThirdPartyTrackingUrl(String urlToTrack, String dartTrackingUrl,
            String thirdPartyTrackingUrlKey) throws CitysearchException {
        try {
            StringBuilder strBuilder = new StringBuilder();
            // dart tracking goes first
            if (dartTrackingUrl != null) {
            ...
            String thirdPartyTrackingUrl = apiProperties.getProperty(thirdPartyTrackingUrlKey);
            if (thirdPartyTrackingUrl != null && thirdPartyTrackingUrl.trim().length() > 0) {
                StringBuilder destinationUrlBuilder = new StringBuilder(thirdPartyTrackingUrl);
                destinationUrlBuilder.append(URLEncoder.encode(urlToTrack, "UTF-8"));
                thirdPartyTrackingUrl = destinationUrlBuilder.toString();
            }
            strBuilder.append(thirdPartyTrackingUrl);
            return strBuilder.toString();
        } catch (UnsupportedEncodingException excep) {
            throw new CitysearchException("Utils", "getTrackingUrl", excep);
        }
