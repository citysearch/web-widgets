...
public abstract class AbstractNearByPlacesFacade {
    ...
    private List<NearbyPlace> toNearbyPlaces(RequestBean request, List<PFPAd> ads)
    ...
    private List<NearbyPlace> toNearbyPlacesBackfill(RequestBean request, List<PFPAd> ads)
            throws CitysearchException {
        List<NearbyPlace> nearbyPlaces = null;
        if (ads != null && !ads.isEmpty()) {
            nearbyPlaces = new ArrayList<NearbyPlace>();
    ...
    protected List<NearbyPlace> getAdsFromPFPLocation(RequestBean request)
        ...
        PFPResponse pfpLocationResponse = pfpProxy.getAdsFromPFPLocation(request, displaySize);
        return toNearbyPlaces(request, pfpLocationResponse.getLocalPfp());
    }

    protected List<NearbyPlace> getAdsFromPFP(RequestBean request, Set<String> listingsToIgnore,
            int requiredNumberOfAds) throws CitysearchException {
        log.info("getAdsFromPFP: Begin");
        if (listingsToIgnore == null) {
            listingsToIgnore = new HashSet<String>();
        }
        PFPResponse pfpResponse = pfpProxy.getAdsFromPFP(request, requiredNumberOfAds,
                listingsToIgnore);
        return toNearbyPlaces(request, pfpResponse.getLocalPfp());
    }

    protected List<NearbyPlace> getNearbyPlacesBackfill(RequestBean request, int noOfBackFillNeeded)
            throws CitysearchException {
        PFPResponse response = pfpProxy.getBackFill(noOfBackFillNeeded);
        List<NearbyPlace> backFillFromPFP = toNearbyPlacesBackfill(request, response.getBackfill());
        return backFillFromPFP;
    }

    protected List<NearbyPlace> getSearchResults(RequestBean request, int maxNoOfResultsRequired)
            throws CitysearchException {
        SearchHelper searchHelper = new SearchHelper(contextPath, maxNoOfResultsRequired);
