...
public abstract class AbstractNearByPlacesFacade {
	...
	protected List<NearbyPlace> getAdsFromPFPLocationCustom(RequestBean request)
			throws CitysearchException {
		log.info("NearbyPlacesHelper.getAdsFromPFPLocationCustom: Begin");
		PFPResponse pfpLocationResponse = pfpProxy.getAdsFromPFPLocationCustom(
				request, displaySize);
		return toNearbyPlaces(request, pfpLocationResponse.getLocalPfp());
	}
